...
router = APIRouter(prefix="/exercise", tags=['Exercise Session'])
...
# Global tracker instance and sessions storage
tracker = ExerciseTracker()
active_sessions = {}

@router.post("/start")
async def start_exercise_session(session_data: SessionStart):
    """Start new exercise session - POST endpoint"""
    tracker.reset()
...
async def analyze_pose(pose_data: PoseData):
    """Analyze pose data from client"""
    return {
        "analysis": "Pose analyzed",
        "feedback": "Keep going",
        "accuracy": 85.5
    }

@router.post("/end/{session_id}")
async def end_exercise_session(session_id: str, session_end: SessionEnd):
    ...
    result = {
        ...
        "total_reps": session_end.total_reps or summary["reps"],
        "max_angle": summary["max_angle"],
        "duration_mins": summary["duration_mins"],
        "accuracy_score": session_end.accuracy_score,
